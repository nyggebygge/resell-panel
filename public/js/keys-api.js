...
class KeysAPIManager {
    ...
    async generateKeys(type, quantity) {
        try {
            this.isLoading = true;
            const response = await window.api.generateKeys({ type, quantity });
            
            if (response.success) {
                this.showNotification('Keys Generated!', `Successfully generated ${quantity} ${type} keys`, 'success');
                ...
                if (window.authManager && window.authManager.currentUser) {
                    window.authManager.currentUser.keysGenerated += quantity;
                    window.authManager.currentUser.credits -= quantity;
                    window.authManager.updateUI();
                }
                
                return response.data;
            } else {
                this.showNotification('Generation Failed', response.message, 'error');
                return null;
            }
        } catch (error) {
            this.showNotification('Generation Error', error.message, 'error');
            return null;
        } finally {
            this.isLoading = false;
        }
